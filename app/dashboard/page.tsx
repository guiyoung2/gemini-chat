...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    void supabase
      ...
      .then(({ data }) => {
        if (data?.plan)
          setSubscription({ plan: data.plan as SubscriptionInfo["plan"] });
      });
  ...
  }, [user, loading, supabase, thisMonth]);
  ...
  const handleSend = useCallback(
      ...
      try {
        ...
        if (savedBotMsg) {
          setMessages((prev) =>
            prev.map((msg) =>
              msg.id === streamingId
                ? { ...msg, id: (savedBotMsg as MessageRow).id }
                : msg,
            ),
          );
        }
        ...
        const aiPreview =
          fullResponse.replace(/\*\*/g, "").slice(0, 45) +
          (fullResponse.length > 45 ? "..." : "");
        setConversations((prev) =>
    ...
    [messages, activeConvId, user, supabase, subscription],
  );
      ...
      <header className="flex items-center justify-end h-14 px-5 bg-[#111111] border-b border-white/6 shrink-0 z-10">
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="flex items-center gap-2">
            ...
            {PLAN_MAX[planKey] === null ? (
              <span className="text-[11px] text-white/30">무제한</span>
            ) : (
              <span className="text-[11px] text-white/40">
                <span
                  className={cn(
                    "font-medium",
                    usageCount >= (PLAN_MAX[planKey] ?? 0)
                      ? "text-red-400"
                      : usageCount >= (PLAN_MAX[planKey] ?? 0) * 0.8
                        ? "text-amber-400"
                        : "text-white/60",
                  )}
                >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-60 shrink-0 bg-[#111111] border-r border-white/6 flex flex-col overflow-hidden">
          ...
          <nav className="flex-1 overflow-y-auto px-2 pb-4 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full">
            {GROUP_ORDER.filter((g) => groupedConversations[g]?.length).map(
              (group) => (
                <div key={group} className="mb-4">
                  <p className="px-3 mb-1 text-[10px] font-medium text-white/25 uppercase tracking-wider">
                    {group}
                  </p>
